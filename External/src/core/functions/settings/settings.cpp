...
void RenderAimMenu() {
    ...
    ImVec2 aLMin = aBase + ImVec2(10.0f,58.0f);
    ImVec2 aLMax = aBase + ImVec2(10.0f+295.0f,58.0f+472.0f);
    ImVec2 aRMin = aBase + ImVec2(315.0f,58.0f);
    ImVec2 aRMax = aBase + ImVec2(315.0f+295.0f,58.0f+472.0f);
    ...
    static float aimScrollL = 0.f;
    static float aimSubScroll[3] = {0.f, 0.f, 0.f};
    float& aimScrollR = aimSubScroll[variables::selectedAimSub];
    if (aHoverL) {
    ...
    float ay = 64.0f - aimScrollL;
    imGuiCustom::Checkbox("Enable Aimbot", &variables::Aimbot::enabled, ImVec2(18.0f, ay));
    imGuiCustom::Keybind("aim_key", &variables::Aimbot::aimbotKey, ImVec2(229.0f, ay - 1.0f), ImVec2(68.0f, 13.0f), &variables::Aimbot::aimbotKeyMode);
    ay += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Show FOV", &variables::Aimbot::showFOV, ImVec2(18.0f, ay));
    imGuiCustom::ColorSquare("fov_color", &variables::Aimbot::fovColor, ImVec2(285.0f, ay + 1.0f));
    ay += imGuiCustom::CheckStep();
    imGuiCustom::SliderFloat("fov_radius", &variables::Aimbot::fovRadius, 10.0f, 500.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "FOV Radius", "%.0f");
    ay += imGuiCustom::SliderTop() + 15.0f;
    if (variables::Aimbot::showFOV) {
        imGuiCustom::Checkbox("Fill FOV", &variables::Aimbot::fillFov, ImVec2(18.0f, ay));
        imGuiCustom::ColorSquare("fov_fill_color", &variables::Aimbot::fovFillColor, ImVec2(285.0f, ay + 1.0f));
        ay += imGuiCustom::CheckStep();
    }
    imGuiCustom::Checkbox("360 Mode", &variables::Aimbot::mode360, ImVec2(18.0f, ay));
    imGuiCustom::Keybind("mode360_key", &variables::Aimbot::mode360Key, ImVec2(229.0f, ay - 1.0f), ImVec2(68.0f, 13.0f), &variables::Aimbot::mode360KeyMode);
    ay += imGuiCustom::CheckStep();
    imGuiCustom::SliderFloat("smoothing", &variables::Aimbot::smoothing, 1.0f, 20.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Smoothing", "%.1f");
    ay += imGuiCustom::SliderTop() + 15.0f;
    const char* targets[] = {"Head", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg", "HumanoidRootPart", "Closest"};
    ay += imGuiCustom::ComboTop();
    imGuiCustom::Combo("aim_target", &variables::Aimbot::aimTarget, targets, 8, ImVec2(18.0f, ay), 279.0f, "Aim Target:");
    ay += imGuiCustom::ComboStep();
    imGuiCustom::Checkbox("Auto Switch Target", &variables::Aimbot::autoSwitch, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Sticky Aim", &variables::Aimbot::stickyAim, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    const char* methods[] = {"Memory", "Viewport", "Raycast", "PF Silent", "Mouse"};
    imGuiCustom::Combo("aim_method", &variables::Aimbot::aimMethod, methods, 5, ImVec2(18.0f, ay + imGuiCustom::ComboTop()), 279.0f, "Aim Method:");
    ay += imGuiCustom::ComboTop() + 22.0f;
    imGuiCustom::Checkbox("Magic Bullet", &variables::Aimbot::magicBullet, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Prediction", &variables::Aimbot::prediction, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    if (variables::Aimbot::prediction) {
        fallen_update_weapon_auto();
        imGuiCustom::Checkbox("Fallen Prediction##fallen_pred", &variables::Aimbot::fallen_prediction, ImVec2(18.0f, ay));
        ay += imGuiCustom::CheckStep();
        if (variables::Aimbot::fallen_prediction) {
            ...
            ay += imGuiCustom::ComboTop();
            imGuiCustom::Combo("fallen_weapon##fallen_weapon", &variables::Aimbot::selected_weapon_index, weapon_items.data(), (int)weapon_items.size(), ImVec2(18.0f, ay), 279.0f, "Weapon:");
            ay += imGuiCustom::ComboStep();
            {
                ...
                wdl->AddText(wfont, wfs, ImVec2(base.x + 18.0f, base.y + ay), imGuiCustom::ColorU32(imGuiCustom::GetTheme().Text), detBuf);
                ay += imGuiCustom::CheckStep();
                ...
                if (ball) {
                    ...
                    wdl->AddText(wfont, wfs, ImVec2(base.x + 18.0f, base.y + ay), imGuiCustom::ColorU32(imGuiCustom::GetTheme().Text), bvBuf);
                    ay += imGuiCustom::CheckStep();
                }
            }
            imGuiCustom::SliderFloat("fallen_bv", &variables::Aimbot::fallen_bv_override, -1.0f, 3000.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Bullet Vel Override (-1=auto)", "%.0f");
            ay += imGuiCustom::SliderTop() + 15.0f;
            imGuiCustom::SliderFloat("fallen_grav", &variables::Aimbot::fallen_grav_mult, 0.0f, 2.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Gravity Mult", "%.2f");
            ay += imGuiCustom::SliderTop() + 15.0f;
        }
    }
    imGuiCustom::Checkbox("Global Prediction", &variables::Aimbot::globalPrediction, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    if (variables::Aimbot::globalPrediction) {
        imGuiCustom::SliderFloat("pred_x", &variables::Aimbot::predX, 0.0f, 10.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Pred X", "%.1f");
        ay += imGuiCustom::SliderTop() + 15.0f;
        imGuiCustom::SliderFloat("pred_y", &variables::Aimbot::predY, 0.0f, 10.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Pred Y", "%.1f");
        ay += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("Include NPC", &variables::Aimbot::includeNPC, ImVec2(18.0f, ay));
    ay += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Silent Tracer", &variables::Aimbot::silentTracer, ImVec2(18.0f, ay));
    imGuiCustom::ColorSquare("tracer_color", &variables::Aimbot::silentTracerColor, ImVec2(285.0f, ay + 1.0f));
    ay += imGuiCustom::CheckStep();
    if (variables::Aimbot::silentTracer) {
        imGuiCustom::SliderFloat("tracer_thick", &variables::Aimbot::silentTracerThickness, 0.5f, 5.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Tracer Thickness", "%.1f");
        ay += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("Prediction Line", &variables::Aimbot::predictionLine, ImVec2(18.0f, ay));
    imGuiCustom::ColorSquare("predline_color", &variables::Aimbot::predictionLineColor, ImVec2(285.0f, ay + 1.0f));
    ay += imGuiCustom::CheckStep();
    if (variables::Aimbot::predictionLine) {
        imGuiCustom::SliderFloat("predline_thick", &variables::Aimbot::predictionLineThickness, 0.5f, 5.0f, ImVec2(18.0f, ay + imGuiCustom::SliderTop()), 279.0f, "Line Thickness", "%.1f");
        ay += imGuiCustom::SliderTop() + 15.0f;
    }
    float contentL = ay + aimScrollL - 64.0f;
    float maxScrollL = contentL - 472.0f + 6.0f; if (maxScrollL < 0.f) maxScrollL = 0.f;
    if (aimScrollL > maxScrollL) aimScrollL = maxScrollL;
    aFg->PopClipRect();
    aFg->PushClipRect(aRMin, aRMax, true);
    float by = 64.0f - aimScrollR;
    if (variables::selectedAimSub == 0) {
    imGuiCustom::Checkbox("Visible Check", &variables::Aimbot::visibleCheck, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Knock Check", &variables::Aimbot::knockCheck, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Forcefield Check", &variables::Aimbot::forcefieldCheck, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Spectate Check", &variables::Aimbot::spectateCheck, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Min Health Check", &variables::Aimbot::healthCheck, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    if (variables::Aimbot::healthCheck) {
        imGuiCustom::SliderFloat("min_health", &variables::Aimbot::minHealth, 0.0f, 100.0f, ImVec2(323.0f, by + imGuiCustom::SliderTop()), 279.0f, "Min Health", "%.0f");
        by += imGuiCustom::SliderTop() + 15.0f;
    }
    }
    if (variables::selectedAimSub == 1) {
    by = 64.0f - aimScrollR;
    imGuiCustom::Checkbox("Spread Modifier", &variables::Aimbot::useSpread, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    if (variables::Aimbot::useSpread) {
        imGuiCustom::SliderFloat("spread_amount", &variables::Aimbot::spreadModifier, 0.0f, 10.0f, ImVec2(323.0f, by + imGuiCustom::SliderTop()), 279.0f, "Spread Amount", "%.2f");
        by += imGuiCustom::SliderTop() + 15.0f;
        imGuiCustom::SliderFloat("hit_chance", &variables::Aimbot::hitChance, 0.0f, 100.0f, ImVec2(323.0f, by + imGuiCustom::SliderTop()), 279.0f, "Hit Chance", "%.0f%%");
        by += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("Deadzone", &variables::Aimbot::useDeadzone, ImVec2(323.0f, by));
    by += imGuiCustom::CheckStep();
    if (variables::Aimbot::useDeadzone) {
        imGuiCustom::SliderFloat("deadzone", &variables::Aimbot::deadzone, 0.0f, 50.0f, ImVec2(323.0f, by + imGuiCustom::SliderTop()), 279.0f, "Deadzone", "%.0f");
        by += imGuiCustom::SliderTop() + 15.0f;
    }
    }
    if (variables::selectedAimSub == 2) {
    by = 64.0f - aimScrollR;
    imGuiCustom::Checkbox("Triggerbot", &variables::Aimbot::triggerbot, ImVec2(323.0f, by));
    imGuiCustom::Keybind("trigger_key", &variables::Aimbot::triggerKey, ImVec2(534.0f, by - 1.0f), ImVec2(68.0f, 13.0f), &variables::Aimbot::triggerKeyMode);
    by += imGuiCustom::CheckStep();
    if (variables::Aimbot::triggerbot) {
        float trigDelayMs = (float)variables::Aimbot::triggerDelay;
        imGuiCustom::SliderFloat("trigger_delay", &trigDelayMs, 0.0f, 500.0f, ImVec2(323.0f, by + imGuiCustom::SliderTop()), 279.0f, "Trigger Delay", "%.0f");
        variables::Aimbot::triggerDelay = (int)trigDelayMs;
        by += imGuiCustom::SliderTop() + 15.0f;
    }
    float contentR = by + aimScrollR - 64.0f;
    float maxScrollR = contentR - 472.0f + 6.0f; if (maxScrollR < 0.f) maxScrollR = 0.f;
...
void RenderVisualMenu() {
    ...
    ImVec2 vLMin = vBase + ImVec2(10.0f,58.0f);
    ImVec2 vLMax = vBase + ImVec2(10.0f+295.0f,58.0f+472.0f);
    ...
    vContent += 0;
    vContent += 10;
    float vMaxScroll = vContent - 472.0f + 6.0f; if(vMaxScroll<0) vMaxScroll=0;
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarSize, 0.0f);
    float ly = 64.0f - variables::ESP::visualScroll;
    imGuiCustom::Checkbox("Enable ESP", &variables::ESP::enabled, ImVec2(18.0f, ly));
    ly += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Boxes", &variables::ESP::boxes, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("box_color", &variables::ESP::boxColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
        if (variables::ESP::boxes) {
            const char* boxModes[] = {"Static", "Dynamic"};
            imGuiCustom::Combo("box_mode", &variables::ESP::boxMode, boxModes, 2, ImVec2(18.0f, ly + imGuiCustom::ComboTop()), 279.0f, "Box Mode:");
            ly += imGuiCustom::ComboTop() + 22.0f;
        imGuiCustom::Checkbox("Box Filled", &variables::ESP::boxFilled, ImVec2(18.0f, ly));
        imGuiCustom::ColorSquare("box_fill_color", &variables::ESP::boxFillColor, ImVec2(285.0f, ly + 1.0f));
        ly += imGuiCustom::CheckStep();
        if (variables::ESP::boxFilled) {
            imGuiCustom::Checkbox("Fill Gradient", &variables::ESP::boxFillGradient, ImVec2(18.0f, ly));
            if (variables::ESP::boxFillGradient)
                imGuiCustom::ColorSquare("box_fill_color2", &variables::ESP::boxFillColor2, ImVec2(285.0f, ly + 1.0f));
            ly += imGuiCustom::CheckStep();
        }
    }
    imGuiCustom::Checkbox("Names", &variables::ESP::names, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("name_color", &variables::ESP::nameColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Distance", &variables::ESP::distance, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("distance_color", &variables::ESP::distanceColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Health Bar", &variables::ESP::healthBar, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("health_color", &variables::ESP::healthColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Tool", &variables::ESP::tool, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("tool_color", &variables::ESP::toolColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Flags", &variables::ESP::flags, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("flags_color", &variables::ESP::flagsColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    if (variables::ESP::flags) {
        const char* flagItems[] = {"State", "Rig", "Health", "Tool", "Distance", "Velocity", "Team", "Role"};
        ly += imGuiCustom::ComboTop();
        imGuiCustom::MultiCombo("flags_sel", variables::ESP::flagSel, flagItems, 8, ImVec2(18.0f, ly), 279.0f, "Flags:");
        ly += imGuiCustom::ComboStep();
    }
    imGuiCustom::Checkbox("Head Dot", &variables::ESP::headDot, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("headdot_color", &variables::ESP::headDotColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    if (variables::ESP::headDot) {
        imGuiCustom::SliderFloat("headdot_size", &variables::ESP::headDotSize, 1.0f, 10.0f, ImVec2(18.0f, ly + imGuiCustom::SliderTop()), 279.0f, "Dot Size", "%.0f");
        ly += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("View Direction", &variables::ESP::viewDirection, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("viewdir_color", &variables::ESP::viewDirColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    if (variables::ESP::viewDirection) {
        imGuiCustom::SliderFloat("viewdir_len", &variables::ESP::viewDirLength, 1.0f, 30.0f, ImVec2(18.0f, ly + imGuiCustom::SliderTop()), 279.0f, "Length", "%.0f");
        ly += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("Skeleton", &variables::ESP::skeleton, ImVec2(18.0f, ly));
    imGuiCustom::ColorSquare("skeleton_color", &variables::ESP::skeletonColor, ImVec2(285.0f, ly + 1.0f));
    ly += imGuiCustom::CheckStep();
    if (variables::ESP::skeleton) {
        imGuiCustom::SliderFloat("skeleton_thick", &variables::ESP::skeletonThickness, 0.5f, 5.0f, ImVec2(18.0f, ly + imGuiCustom::SliderTop()), 279.0f, "Skeleton Thickness", "%.1f");
        ly += imGuiCustom::SliderTop() + 15.0f;
    }
    imGuiCustom::Checkbox("Mesh Chams", &variables::ESP::meshChams, ImVec2(18.0f, ly));
    ly += imGuiCustom::CheckStep();
    if (variables::ESP::meshChams) {
        ...
        chamsShaderType = 0;
        ly += imGuiCustom::ComboTop();
        imGuiCustom::Combo("chams_mesh", &chamsMeshFlat, flatOnly, 1, ImVec2(18.0f, ly), 279.0f, "Mesh:");
        ly += imGuiCustom::ComboStep();
        ly += imGuiCustom::ComboTop();
        imGuiCustom::Combo("chams_shadertype", &chamsShaderType, shaderOnly, 1, ImVec2(18.0f, ly), 279.0f, "Shaders type:");
        ly += imGuiCustom::ComboStep();
        ly += imGuiCustom::ComboTop();
        imGuiCustom::Combo("chams_shaders", &variables::ESP::meshChamsDxMode, Cheat::Visuals::MeshDxShader::ModeNames(), Cheat::Visuals::MeshDxShader::ModeNameCount(), ImVec2(18.0f, ly), 279.0f, "Shaders:");
        ly += imGuiCustom::ComboStep();
    ...
    vFg->PopClipRect();
    ImGui::PopStyleVar();
    if (variables::selectedVisualSub == 0) {
    float sy = 64.0f;
    imGuiCustom::Checkbox("Dead Check", &variables::ESP::deadCheck, ImVec2(323.0f, sy));
    sy += imGuiCustom::CheckStep();
    imGuiCustom::Checkbox("Local Player", &variables::ESP::localPlayer, ImVec2(323.0f, sy));
    }

    if (variables::selectedVisualSub == 1)
    {
        ImVec2 base = ImGui::GetWindowPos();
        ImVec2 pMin = base + ImVec2(315.0f + imGuiCustom::g_contentOffset.x, 58.0f + imGuiCustom::g_contentOffset.y);
        ImVec2 pMax = pMin + ImVec2(295.0f, 472.0f);
        ...
        auto contentH = [&]()->float{
            ...
            h += imGuiCustom::CheckStep() * 3;
            if (variables::World::crates) h += imGuiCustom::ComboStep() + imGuiCustom::ComboTop();
            if (variables::World::keycards) h += imGuiCustom::ComboStep() + imGuiCustom::ComboTop();
            return h + 10.0f;
        }();
        float maxScroll = contentH - 472.0f + 6.0f;
        ...
        float wy = 64.0f - variables::World::worldScroll;

        ImGui::GetStyle().ScrollbarSize = 0.0f;
        imGuiCustom::Checkbox("Enable world##world_enabled", &variables::World::enabled, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::enabled) {
            imGuiCustom::Checkbox("Name##world_name", &variables::World::name, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
            imGuiCustom::Checkbox("Distance##world_distance", &variables::World::distance, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
        }
        imGuiCustom::Checkbox("Ores##world_ores", &variables::World::ores, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::ores) {
            const char* oreItems[] = {"Stone", "Phosphate", "Metal"};
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_ores_sel##world_ores", variables::World::oresSel, oreItems, 3, ImVec2(323.0f, wy), 279.0f, "Ores:");
            wy += imGuiCustom::ComboStep();
        }
        imGuiCustom::Checkbox("Plants##world_plants_cb", &variables::World::plants, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::plants) {
            ...
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_plants_sel##world_plants", variables::World::plantsSel, plantItems, 7, ImVec2(323.0f, wy), 279.0f, "Plants:");
            wy += imGuiCustom::ComboStep();
        }
        imGuiCustom::Checkbox("Animals##world_animals", &variables::World::animals, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::animals) {
            const char* animalItems[] = {"Deer", "WildBoar", "Wolf"};
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_animals_sel##world_animals", variables::World::animalsSel, animalItems, 3, ImVec2(323.0f, wy), 279.0f, "Animals:");
            wy += imGuiCustom::ComboStep();
            imGuiCustom::Checkbox("Box##world_animals_box", &variables::World::animalsBox, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
            imGuiCustom::Checkbox("Health##world_animals_health", &variables::World::animalsHealth, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
        }
        imGuiCustom::Checkbox("Soldiers##world_soldiers", &variables::World::soldiers, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::soldiers) {
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_soldiers_sel##world_soldiers", variables::World::soldiersSel, WorldCache::kSoldierNames, WorldCache::kSoldierCount, ImVec2(323.0f, wy), 279.0f, "Soldiers:");
            wy += imGuiCustom::ComboStep();
            imGuiCustom::Checkbox("Box##world_soldiers_box", &variables::World::soldiersBox, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
            imGuiCustom::Checkbox("Health##world_soldiers_health", &variables::World::soldiersHealth, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
        }
        imGuiCustom::Checkbox("Tools##world_tools", &variables::World::tools, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::tools) {
            ...
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_tools_sel##world_tools", variables::World::toolsSel, toolItems, 7, ImVec2(323.0f, wy), 279.0f, "Tools:");
            wy += imGuiCustom::ComboStep();
            imGuiCustom::Checkbox("3D Box##world_tools_box", &variables::World::toolsBox, ImVec2(323.0f, wy));
            wy += imGuiCustom::CheckStep();
        }
        imGuiCustom::Checkbox("Crates##world_crates", &variables::World::crates, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::crates) {
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_crates_sel##world_crates", variables::World::cratesSel, WorldCache::kCrateNames, WorldCache::kCrateCount, ImVec2(323.0f, wy), 279.0f, "Crates:");
            wy += imGuiCustom::ComboStep();
        }
        imGuiCustom::Checkbox("Drops##world_drops", &variables::World::drops, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        imGuiCustom::Checkbox("Keycards##world_keycards", &variables::World::keycards, ImVec2(323.0f, wy));
        wy += imGuiCustom::CheckStep();
        if (variables::World::keycards) {
            const char* keycardItems[] = {"Yellow", "Purple", "Red", "Pink", "Black"};
            wy += imGuiCustom::ComboTop();
            imGuiCustom::MultiCombo("world_keycards_sel##world_keycards", variables::World::keycardsSel, keycardItems, 5, ImVec2(323.0f, wy), 279.0f, "Keycards:");
            wy += imGuiCustom::ComboStep();
        }
        fg->PopClipRect();
        ImGui::PopStyleVar();
    }
    if (variables::selectedVisualSub == 2)
    {
        ImVec2 base = ImGui::GetWindowPos();
        static float s_qContent = 120.0f;
        ImVec2 qMin = base + ImVec2(315.0f + imGuiCustom::g_contentOffset.x, 58.0f + imGuiCustom::g_contentOffset.y);
        ImVec2 qMax = qMin + ImVec2(295.0f, 472.0f);
        ImVec2 mp = ImGui::GetIO().MousePos;
        ...
        float qMaxScroll = s_qContent - 472.0f + 6.0f;
        ...
        float qy = 64.0f - variables::World::lightScroll;
        ImGui::GetStyle().ScrollbarSize = 0.0f;
        auto qLabel = [&](const char* text, float rowY) {
            ...
            qg->AddText(lfont, lfs, ImVec2(base.x + 323.0f + imGuiCustom::g_contentOffset.x, base.y + rowY + 2.0f + imGuiCustom::g_contentOffset.y), imGuiCustom::ColorU32(imGuiCustom::GetTheme().Text), text);
        };
        imGuiCustom::Checkbox("Clock Time##light_clock", &variables::World::clockTimeEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::clockTimeEnabled) {
            imGuiCustom::SliderFloat("light_clocktime", &variables::World::clockTimeValue, 0.0f, 24.0f, ImVec2(323.0f, qy + imGuiCustom::SliderTop()), 279.0f, "Time", "%.1f");
            qy += imGuiCustom::SliderTop() + 15.0f;
        }
        imGuiCustom::Checkbox("Brightness##light_bright", &variables::World::brightnessEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::brightnessEnabled) {
            imGuiCustom::SliderFloat("light_brightness", &variables::World::brightnessValue, 0.0f, 5.0f, ImVec2(323.0f, qy + imGuiCustom::SliderTop()), 279.0f, "Brightness", "%.2f");
            qy += imGuiCustom::SliderTop() + 15.0f;
        }
        imGuiCustom::Checkbox("Ambient##light_amb", &variables::World::ambientEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::ambientEnabled) {
            qLabel("Ambient", qy);
            imGuiCustom::ColorSquare("light_ambientcolor", &variables::World::ambientColor, ImVec2(590.0f, qy + 1.0f));
            qy += imGuiCustom::CheckStep();
            qLabel("Outdoor", qy);
            imGuiCustom::ColorSquare("light_outdoorcolor", &variables::World::outdoorAmbientColor, ImVec2(590.0f, qy + 1.0f));
            qy += imGuiCustom::CheckStep();
        }
        imGuiCustom::Checkbox("Fog##light_fog", &variables::World::fogEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::fogEnabled) {
            qLabel("Fog", qy);
            imGuiCustom::ColorSquare("light_fogcolor", &variables::World::fogColor, ImVec2(590.0f, qy + 1.0f));
            qy += imGuiCustom::CheckStep();
            imGuiCustom::SliderFloat("light_fogstart", &variables::World::fogStart, 0.0f, 10000.0f, ImVec2(323.0f, qy + imGuiCustom::SliderTop()), 279.0f, "Fog Start", "%.0f");
            qy += imGuiCustom::SliderTop() + 15.0f;
            imGuiCustom::SliderFloat("light_fogend", &variables::World::fogEnd, 0.0f, 100000.0f, ImVec2(323.0f, qy + imGuiCustom::SliderTop()), 279.0f, "Fog End", "%.0f");
            qy += imGuiCustom::SliderTop() + 15.0f;
        }
        imGuiCustom::Checkbox("Exposure##light_exp", &variables::World::exposureEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::exposureEnabled) {
            imGuiCustom::SliderFloat("light_exposure", &variables::World::exposureValue, -5.0f, 5.0f, ImVec2(323.0f, qy + imGuiCustom::SliderTop()), 279.0f, "Exposure", "%.2f");
            qy += imGuiCustom::SliderTop() + 15.0f;
        }
        imGuiCustom::Checkbox("Skybox##light_sky", &variables::World::skyboxEnabled, ImVec2(323.0f, qy));
        qy += imGuiCustom::CheckStep();
        if (variables::World::skyboxEnabled) {
            const char* skyPresets[] = {"Clear", "Custom"};
            qy += imGuiCustom::ComboTop();
            imGuiCustom::Combo("light_skypreset", &variables::World::skyboxPreset, skyPresets, 2, ImVec2(323.0f, qy), 279.0f, "Preset:");
            qy += imGuiCustom::ComboStep();
            if (variables::World::skyboxPreset == 1) {
                static char skyIds[1024] = {};
                ImVec2 wpos = ImGui::GetWindowPos();
                ImGui::PushID("light_skyids");
                ImGui::SetCursorScreenPos(ImVec2(wpos.x + 323.0f + imGuiCustom::g_contentOffset.x, wpos.y + qy + imGuiCustom::g_contentOffset.y));
                ImGui::PushItemWidth(279.0f);
        ...
        s_qContent = qy + variables::World::lightScroll - 64.0f + 10.0f;
        qg->PopClipRect();
        ImGui::PopStyleVar();
    }
}
...
void RenderSettingsMenu() {
    float gy = 64.0f;
    imGuiCustom::Checkbox("VSync", &variables::Misc::vsync, ImVec2(18.0f, gy));
    gy += imGuiCustom::CheckStep();
    float fps = (float)variables::Misc::fpsLimit;
    imGuiCustom::SliderFloat("fps_limit", &fps, 0.0f, 240.0f, ImVec2(18.0f, gy + imGuiCustom::SliderTop()), 279.0f, fps < 0.5f ? "FPS Limit: Unlimited" : "FPS Limit", "%.0f");
    ...
    if (imGuiCustom::Combo("priority", &variables::Misc::priority, priorities, 4, ImVec2(18.0f, gy), 279.0f, "Priority:")) {
    ...
    {
        if (imGuiCustom::ButtonPos("Explorer##open_exp", ImVec2(18.0f, gy), ImVec2(279.0f, 20.0f), false))
            Explorer::SetOpen(!Explorer::IsOpen());
        gy += 26.0f;
        if (imGuiCustom::ButtonPos("Players##open_players", ImVec2(18.0f, gy), ImVec2(279.0f, 20.0f), false))
            PlayersWidget::SetOpen(!PlayersWidget::IsOpen());
        gy += 26.0f;
        if (imGuiCustom::ButtonPos("Hotbar##open_back", ImVec2(18.0f, gy), ImVec2(279.0f, 20.0f), false))
...

    {
        ...
        cdl->AddText(cfont, cfs, ImVec2(base.x+18.0f, base.y+gy+2.0f), imGuiCustom::ColorU32(imGuiCustom::GetTheme().TextBright), "Config");
        gy += 18.0f;
        ...
        auto buildStr = []()->std::string{
            ...
            for(int k=0;k<WorldCache::kSoldierCount;++k) b((std::string("World.soldiersSel")+std::to_string(k)).c_str(), variables::World::soldiersSel[k]);
            ...
            b("World.crates", variables::World::crates);
            for(int k=0;k<WorldCache::kCrateCount;++k) b((std::string("World.cratesSel")+std::to_string(k)).c_str(), variables::World::cratesSel[k]);
            for(int k=0;k<WorldCache::kCrateCount;++k) c((std::string("World.cratesColor")+std::to_string(k)).c_str(), variables::World::cratesColor[k]);
            b("World.drops", variables::World::drops);
            b("World.keycards", variables::World::keycards);
            for(int k=0;k<5;++k) b((std::string("World.keycardsSel")+std::to_string(k)).c_str(), variables::World::keycardsSel[k]);
        ...
        auto parseStr = [](const std::string& s){
            ...
            static const auto table = [&](){
                ...
                Ic("Aimbot.aimMethod", variables::Aimbot::aimMethod, 0, 4);
                B("Aimbot.magicBullet", variables::Aimbot::magicBullet);
                I("Aimbot.aimbotKey", variables::Aimbot::aimbotKey);
            ...
            auto selIdx = [](const std::string& key, const char* pre, int n)->int{
                std::string ps(pre);
                if(key.rfind(ps,0)!=0 || key.size()<=ps.size()) return -1;
                int d = 0;
                for(size_t i=ps.size();i<key.size();++i){
            ...
            while(std::getline(iss,line)){
                ...
                selB(key, "World.soldiersSel", variables::World::soldiersSel, WorldCache::kSoldierCount, val);
                ...
                selB(key, "World.cratesSel", variables::World::cratesSel, WorldCache::kCrateCount, val);
                selC(key, "World.cratesColor", variables::World::cratesColor, WorldCache::kCrateCount, val);
                selB(key, "World.keycardsSel", variables::World::keycardsSel, 5, val);
            }
        };
        ImVec2 inpPos = base + ImVec2(18.0f, gy + imGuiCustom::g_contentOffset.y);
...

        {
            ...
            if (!cfgPtrs.empty()) {
                gy += imGuiCustom::ComboTop();
                if (imGuiCustom::Combo("cfg_select##cfg_select", &cfgSel, cfgPtrs.data(), (int)cfgPtrs.size(), ImVec2(18.0f, gy), 279.0f, "Configs:")) {
...

    {
        ...
        if (warn)
            edl->AddImage(warn, ImVec2(base.x + 18.0f, base.y + gy + 1.0f), ImVec2(base.x + 18.0f + iconSz, base.y + gy + 1.0f + iconSz), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 70, 70, 255));
        imGuiCustom::Checkbox("Wireframe##settings_wireframe", &variables::World::wireframe, ImVec2(warn ? 36.0f : 18.0f, gy));
        gy += imGuiCustom::CheckStep();
        imGuiCustom::Checkbox("Player Preview", &variables::Aimbot::playerPreview, ImVec2(18.0f, gy));
        gy += imGuiCustom::CheckStep();
    }
    float hy = 64.0f;
    if (variables::selectedSettingsSub == 0) {
    imGuiCustom::SliderFloat("menu_font", &variables::Misc::menuFontSize, 0.8f, 1.5f, ImVec2(323.0f, hy + imGuiCustom::SliderTop()), 279.0f, "Menu Font Size", "%.2f");
    ...
    imGuiCustom::SliderFloat("esp_font", &variables::Misc::espFontSize, 10.0f, 20.0f, ImVec2(323.0f, hy + imGuiCustom::SliderTop()), 279.0f, "ESP Font Size", "%.0f");
    ...
    auto themeRow = [&](const char* id, const char* label, ImVec4* col) {
        ...
        dl->AddText(font, fs, ImVec2(base.x + 323.0f, base.y + ty), imGuiCustom::ColorU32(imGuiCustom::GetTheme().Text), label);
        imGuiCustom::ColorSquare(id, col, ImVec2(590.0f, ty + 1.0f));
        ty += imGuiCustom::CheckStep();
    };
    if (variables::selectedSettingsSub == 1) {
    ...
    if (variables::selectedSettingsSub == 2) {
    themeRow("theme_text", "Text", &variables::Theme::text);
    themeRow("theme_textbright", "Text Bright", &variables::Theme::textBright);
    if (imGuiCustom::ButtonPos("Reset Theme##reset_th", ImVec2(323.0f, ty), ImVec2(279.0f, 20.0f))) {
